...
const Events = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        // For demonstration, we'll use the mock data since our actual events table might not have all fields
        // In a real app, you'd fetch from the events table and do proper formatting
        ...
        if (user) {
          ...
          if (!error && registrations) {
            setRegisteredEvents(registrations.map(reg => reg.event_id));
          }
        }
  ...
  const registerForEvent = async (eventId: string) => {
    ...
    try {
      ...
      toast({
        ...
        description: error.message || "There was a problem registering for the event.",
        variant: "destructive",
      });
    }
  };
                      ...
                      <div className="col-span-2 p-6 flex flex-col">
                        ...
                        <div className="flex flex-wrap gap-3 items-center mt-auto">
                          <Button 
                            ...
                            disabled={!user || registeredEvents.includes(event.id)}
                            className={`${
